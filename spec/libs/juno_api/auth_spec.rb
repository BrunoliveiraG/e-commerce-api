...
describe JunoApi::Auth do
  ...
  context 'when calling the #singleton' do
    let(:response) do
      double(
  ...
  context 'when call #access_token' do
    ...
    it 'returns the same access token before expiring' do
    ...
    it 'returns another access token when it expires' do
      first_auth = auth_class.singleton
      travel 3.days do
        second_auth = auth_class.singleton
    ...
    it 'returns another access token when it reaches expiration date limit percentage' do
      first_auth = auth_class.singleton

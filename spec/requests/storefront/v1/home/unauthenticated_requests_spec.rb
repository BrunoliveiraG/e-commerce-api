...
RSpec.describe "Storefront V1 Home", type: :request do
  context "GET /home" do
    ...
    let!(:latest_released_products) do
      products = []
      10.times do
    ...
    context "on :featured" do
      ...
      it "does not returns any non-featured products" do
        get url, headers: unauthenticated_header
        unexpected_products = (latest_released_products + cheap_products).map do |product|
    ...
    context "on :latest_releases" do
      ...
      it "returns random latest released products" do
        ...
        expect(body_json['latest_releases']).to satisfy do |products|
          products & expected_products == products
        end
      end
      ...
      it "does not returns any non-latest released products" do
        ...
        expect(body_json['latest_releases']).to_not include *unexpected_products
      end
    end

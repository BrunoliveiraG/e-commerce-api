module Admin::Dashboard
  class TopFiveProductsService
    ...
    def search_top_five
      ...
      Product.joins(line_items: :order).merge(range_date_orders).group(:id).order('total_sold DESC, total_qty DESC').limit(NUMBER_OF_RECORDS).select(:id, :name, line_item_arel[:sold].as('total_sold'), line_item_arel[:quantity].as('total_qty'))
    end
